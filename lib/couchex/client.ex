        ...
        case Map.to_list(view_opts) do
          ...
          [{:key_based, true}] ->
            case opts["include_docs"] do
              nil ->
                {:ok,
                 Enum.reduce(res["rows"], %{}, fn x, acc -> Map.put(acc, x["key"], x["value"]) end)}

              true ->
                {:ok,
                 Enum.reduce(res["rows"], %{}, fn x, acc -> Map.put(acc, x["key"], x["doc"]) end)}
            end
